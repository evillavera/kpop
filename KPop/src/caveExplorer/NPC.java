 ...
 */
	...
	public NPC() {
		...
		//to indicate the NPC does't have a position yet, use coordinates -1,-1
		this.currentCol = -1;
		this.currentRow = -1;
	...
	public void setPosition(int row, int col) {
		if(row >= 0 && row < floor.length && 
				col >= 0 && col < floor[row].length && 
				floor[row][col] instanceof NPCRoom) {
			//remove the npc from current room
			if(currentRoom != null) {
				//need this because cannot call a method on something that does not exist
				//if statement only happens if currentRoom exists
